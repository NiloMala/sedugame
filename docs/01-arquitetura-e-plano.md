...
Escopo do MVP, público e volume de conteúdo seguem exatamente a seção 50 do brief (3 campanhas, 6º/7º anos, ~120 questões). **Atualização 2026-08-06**: seed de demonstração expandido de 1 para 3 campanhas (`CaraguatatubaCampaignSeeder` + `DemoCampaignsExpansionSeeder`, ~18 questões ao todo) — pedido explicitamente pelo cliente pra uma apresentação, ainda não acesso de aluno real. Regra seguida ao escrever o conteúdo novo: só fato de conhecimento público bem estabelecido e fácil de verificar (agrupamento dos 4 municípios do Litoral Norte de SP, Ilhabela ser insular, relevo/clima em termos gerais, bioma Mata Atlântica) — **nenhuma** data específica, estatística de população ou afirmação histórica pontual, exatamente o tipo de dado que precisa de fonte oficial da Secretaria pra afirmar com segurança. Continua **não sendo** o conteúdo final de produção nem as ~120 questões do MVP — segue precisando de revisão pedagógica antes de qualquer uso com alunos reais.
